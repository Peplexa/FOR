...
#same as the best extractor, but randomly adds some modifiers to 3% of the feature files
...
def extract_features(audio_path, n_fft=1024):
    ...
    try:
        ...
        # Check for the 3% chance condition
        if random.random() < 0.03:
            # Decide between adding silence or slowing down
            x =random.random()
            if x < 0.33:
                # Slow down the audio by 10%
                # Randomly stretch or compress the audio time
                stretch_rate = random.uniform(0.8, 1.2)  # Choose a random rate
                audio = librosa.effects.time_stretch(audio, rate=stretch_rate)
            elif x< .66:
                    # Add Gaussian noise
                    noise_intensity = 0.005  # Adjust based on the desired intensity
                    noise = np.random.normal(0, noise_intensity, audio.shape)
                    audio += noise

            else:
                # Insert silence randomly
                silence_length = int(0.01 * len(audio))  # 1% of the audio length
                silence_points = random.sample(range(len(audio)), 3)  # Choose 3 random points
                for point in silence_points:
                    start_index = max(0, point - silence_length // 2)
                    end_index = start_index + silence_length
                    audio[start_index:end_index] = 0  # Insert silence
...
def process_and_save_dataset(root_dir):
    ...
    save_dataset(features_train, labels_train, './npyAdversarial/training_features.npy', './npyAdversarial/training_labels.npy')
    save_dataset(features_val, labels_val, './npyAdversarial/validation_features.npy', './npyAdversarial/validation_labels.npy')
    save_dataset(features_test, labels_test, './npyAdversarial/testing_features.npy', './npyAdversarial/testing_labels.npy')
...
root_dir = '.'  # Adjust this path to your dataset directory, mine was saved in the same folder
