...
#This one uses the adversarial npy
...
X_train, y_train = load_dataset('./npyAdversarial/training_features.npy', './npyAdversarial/training_labels.npy')
X_test, y_test = load_dataset('./npyAdversarial/testing_features.npy', './npyAdversarial/testing_labels.npy')
...
model_filename = './forest2.joblib'  # Change to 'forest2.joblib'
dump(rf_classifier, model_filename)
end_time = time.time()  # End timing
time_taken = end_time - start_time  # Calculate the time taken
print(f"Model saved to {model_filename}")
print(f"Time taken to train and save the model: {time_taken} seconds")
average_time_per_file = time_taken / 1  # Since we are saving only one file, the average is the total time
print(f"Average time taken per file: {average_time_per_file} seconds")
